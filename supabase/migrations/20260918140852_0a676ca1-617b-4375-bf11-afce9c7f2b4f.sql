-- Modulo Previsao de Arrecadacao

-- Serie historica unificada (fonte unica de verdade, substitui as duas planilhas manuais)
...
-- Valores futuros/projetados das variaveis exogenas (cenarios para prever alem do dado real)
...
-- Execucoes de previsao (uma linha por rodada solicitada, acompanha o job assincrono no GitHub Actions)
...
-- Nao ha policy de UPDATE para "authenticated": o status/resultado so e' escrito
-- pela Edge Function (via service_role), nunca diretamente pelo usuario.
